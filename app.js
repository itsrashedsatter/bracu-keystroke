/* ═══════════════════════════════════════════
   ...
   SERVER VERSION — posts data to Flask backend
   ═══════════════════════════════════════════ */
...
(function () {
  ...
  const inputTargetAttempts  = $('input-target-attempts');
  ...
  navBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (target === 'panel-subjects') renderSubjects();
    });
  });
  ...
  selectPrompt.addEventListener('change', () => {
    customPromptGroup.classList.toggle('hidden', selectPrompt.value !== 'custom');
  });

  // ─── Progress Preview on User ID input ───
    ...
    progressPreview.innerHTML = `
      <div style="width:100%">
        <div style="display:flex;justify-content:space-between;margin-bottom:4px;font-size:13px">
          <span>${totalAttempts} / ${target} attempts</span><span>${pct}%</span>
        </div>
  ...
  btnStart.addEventListener('click', () => {
    ...
    if (!userId) { toast('Please enter a Subject ID.', 'error'); inputUserId.focus(); return; }

    const promptVal = selectPrompt.value;
    let prompt = promptVal;
    if (promptVal === 'custom') {
      prompt = inputCustomPrompt.value.trim();
      if (!prompt) { toast('Enter a custom prompt.', 'error'); inputCustomPrompt.focus(); return; }
    }
  ...
  btnSubmit.addEventListener('click', () => {
    if (state.keystrokes.length === 0) { toast('No keystrokes captured yet.', 'error'); return; }
  ...
  function saveCurrentAttempt() {
    ...
    // ── Send this attempt's keylog to server ──
    sendToServer(
      ...
      if (saved > 0) {
        console.log(`✅ ${saved} keystrokes saved to server`);
      }
    });
  ...
  typingInput.addEventListener('keydown', e => {
    ...
    // ── Store raw event for server ──
    state.keyEvents.push({ type: 'keydown', key: e.key, code: e.code, time: now });
  ...
  typingInput.addEventListener('keyup', e => {
    ...
    // ── Store raw event for server ──
    state.keyEvents.push({ type: 'keyup', key: e.key, code: e.code, time: now });
    ...
    const row = {
      index:       state.keystrokes.length + 1,
      key:         e.key,
      code:        e.code,
    ...
    if (!state.currentSession.isFree) {
      ...
      if (typed.length <= expected.length) {
        if (typed[typed.length - 1] !== expected[typed.length - 1] && e.key.length === 1)
          state.errorCount++;
      }
    }
  ...
  function addLiveRow(row) {
    ...
    tr.innerHTML = `
      <td>${row.index}</td><td>${escHtml(displayKey(row.key))}</td>
      <td>${escHtml(row.code)}</td><td>${row.pressTime}</td>
      <td>${row.releaseTime}</td><td>${row.dwell}</td>
      <td>${row.flight}</td><td>${row.ikl}</td>
      <td>${row.dd}</td><td>${row.uu}</td>`;
  ...
  function displayKey(key) {
    const map = { ' ': '␣', Enter: '⏎', Backspace: '⌫', Shift: '⇧', Tab: '⇥', CapsLock: '⇪', Control: 'Ctrl', Alt: 'Alt', Meta: '⌘' };
    return map[key] || key;
  }
  ...
  function updateMetrics() {
    ...
    if (n === 0) {
      ['metric-dwell','metric-flight','metric-ikl','metric-wpm'].forEach(id => $(id).textContent = '—');
      ['bar-dwell','bar-flight','bar-ikl','bar-wpm','bar-keystrokes','bar-errors'].forEach(id => setBar(id, 0));
      return;
    }
  ...
  function renderSubjects() {
    ...
    state.sessions.forEach(s => {
      if (!subjectMap[s.userId]) subjectMap[s.userId] = { attempts: 0, lastActive: s.startedAt };
      subjectMap[s.userId].attempts += s.attempts.length;
      if (s.startedAt > subjectMap[s.userId].lastActive) subjectMap[s.userId].lastActive = s.startedAt;
    });
    ...
    const totalAtt   = subjects.reduce((s, [, d]) => s + d.attempts, 0);

    const circumference = 326.73;
    const ring = $('overview-ring-progress');
    const ringGoal = parseInt($('overview-target')?.textContent) || 50;
    if (ring) ring.style.strokeDashoffset = circumference * (1 - Math.min(completed / ringGoal, 1));
    const countEl = $('overview-complete-count');
    if (countEl) countEl.textContent = completed;

    [['ov-total-subjects', subjects.length], ['ov-complete-subjects', completed],
     ['ov-in-progress', inProgress], ['ov-total-attempts', totalAtt]]
    ...
    subjects.sort((a, b) => b[1].attempts - a[1].attempts).forEach(([uid, data]) => {
      ...
      tr.innerHTML = `
        ...
        <td>${done ? '✅ Complete' : data.attempts > 0 ? '🔄 In Progress' : '⬜ Not Started'}</td>
  ...
  function renderSessions() {
    ...
    list.querySelectorAll('[data-delete]').forEach(btn => {
      btn.addEventListener('click', () => {
        state.sessions.splice(parseInt(btn.dataset.delete), 1);
        persistSessions(); renderSessions(); updateHeaderStats();
  ...
  $('btn-clear-all').addEventListener('click', () => {
    ...
    state.sessions = []; persistSessions(); renderSessions(); updateHeaderStats();
    ...
    importInput.addEventListener('change', e => {
      ...
      reader.onload = ev => {
        try {
          ...
          state.sessions.push(...imported); persistSessions();
          renderSessions(); updateHeaderStats();
          toast(`Imported ${imported.length} session(s).`, 'success');
        } catch { toast('Import failed — invalid JSON file.', 'error'); }
        importInput.value = '';
      };
  ...
  $('btn-export-raw').addEventListener('click', () => {
    ...
    const rows = [];
    state.sessions.forEach(s => s.attempts.forEach(a => a.keystrokes.forEach(k => {
      rows.push([csvSafe(s.userId), csvSafe(s.id), a.attempt,
        csvSafe(displayKey(k.key)), csvSafe(k.code),
        k.pressTime, k.releaseTime, k.dwell, k.flight, k.ikl, k.dd, k.uu].join(','));
    })));
  ...
  $('btn-export-features').addEventListener('click', () => {
    ...
    const rows = [];
    state.sessions.forEach(s => s.attempts.forEach(a => {
      const ks = a.keystrokes; if (!ks.length) return;
  ...
  $('btn-export-dsl').addEventListener('click', () => {
    if (state.sessions.length === 0) { toast('No data to export.', 'error'); return; }
    const rows = []; const allHeaders = new Set(); const attemptData = [];
    state.sessions.forEach(s => s.attempts.forEach(a => {
      const ks = a.keystrokes; if (ks.length < 2) return;
      const record = { subject: s.userId, sessionIndex: s.id, rep: a.attempt };
      ks.forEach((k, i) => {
        const kname = sanitizeKeyName(k.key);
        const hKey  = 'H.' + kname;
        record[hKey] = k.dwell; allHeaders.add(hKey);
        if (i > 0) {
          const pname = sanitizeKeyName(ks[i - 1].key);
          record['DD.' + pname + '.' + kname] = k.dd;   allHeaders.add('DD.' + pname + '.' + kname);
          record['UD.' + pname + '.' + kname] = k.flight; allHeaders.add('UD.' + pname + '.' + kname);
        }
      });
      attemptData.push(record);
    }));
    const sortedHeaders = ['subject', 'sessionIndex', 'rep', ...Array.from(allHeaders).sort()];
    attemptData.forEach(rec => rows.push(sortedHeaders.map(h => rec[h] ?? '').join(',')));
  ...
  function updateSummary() {
    const subjects = new Set(); let totalAtt = 0, totalKs = 0;
    ...
    state.sessions.forEach(s => {
      ...
      s.attempts.forEach(a => {
        ...
        a.keystrokes.forEach(k => {
          totalKs++; allDwells.push(k.dwell);
    ...
    $('sum-attempts').textContent   = totalAtt;
    ...
    // ── Show server download link ──
    const existing = $('server-download-link');
    if (!existing) {
      const summaryGrid = $('summary-grid');
      if (summaryGrid) {
        const div = document.createElement('div');
        div.id = 'server-download-link';
        div.style.cssText = 'grid-column:1/-1;margin-top:16px;text-align:center';
        div.innerHTML = `
          <a href="${SERVER_URL}/download" target="_blank"
            ...
            ⬇️ Download ALL Server Data (CSV)
          </a>
          <p style="margin-top:8px;font-size:12px;color:var(--text-muted)">
            Downloads all participant data saved on the server
          </p>`;
        summaryGrid.parentElement.appendChild(div);
      }
    }
  }
  ...
  function stats(arr) {
    ...
    return [mean(arr).toFixed(2), std(arr).toFixed(2), Math.min(...arr).toFixed(2), Math.max(...arr).toFixed(2)];
  }

  function sanitizeKeyName(key) {
    const map = { '.':'period',',':'comma',' ':'space','!':'exclam','@':'at','#':'hash','$':'dollar','%':'percent','^':'caret','&':'ampersand','*':'asterisk','(':'lparen',')':'rparen','-':'minus','=':'equal','+':'plus','/':'slash','\\':'backslash',"'":'apostrophe','"':'quote',';':'semicolon',':':'colon','[':'lbracket',']':'rbracket','{':'lbrace','}':'rbrace','<':'less','>':'greater','?':'question','`':'backtick','~':'tilde','_':'underscore','|':'pipe',Enter:'Return',Backspace:'BackSpace',Shift:'Shift',Control:'Control',Alt:'Alt',Tab:'Tab',CapsLock:'CapsLock',Meta:'Meta' };
    return map[key] || key;
  }
